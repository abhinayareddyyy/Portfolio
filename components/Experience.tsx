...
export default function Experience() {
  return (
    <section id="experience" className="py-10 bg-[#111111] relative overflow-hidden">
            ...
            <div className="space-y-4">
              {portfolioData.experience.map((exp) => (
                <motion.div key={exp.id} variants={itemVariants} className="relative pl-24">
                  ...
                  <motion.div
                      className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-5 lg:p-6 hover:border-[#F70000]/60 hover:shadow-lg hover:shadow-[#F70000]/10 transition-all duration-300"
                    whileHover={{ x: 4 }}
                  >
                    <div className="flex items-start justify-between mb-2 flex-wrap gap-2">
                      <div>
                        <h3 className="text-2xl font-bold text-[#E6E6E5]">{exp.company}</h3>
                    ...
                    <ul className="space-y-2 mt-4">
                      {exp.details.map((detail, idx) => (
                        <li key={idx} className="flex gap-3 text-[#A7A7A7] text-base leading-relaxed">
